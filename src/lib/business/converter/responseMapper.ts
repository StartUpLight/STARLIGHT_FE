...
const convertContentItemToEditorJson = (item: BlockContentItem): JSONNode[] => {
  if (item.type === 'text') {
    ...
    const hasHeader =
      Array.isArray(tableItem.columns) && tableItem.columns.length > 0;
    if (hasHeader) {
    ...
    if (tableItem.rows && tableItem.rows.length > 0) {
      ...
      if (hasHeader) {
        const firstEqualsHeader =
          ...
          dataRows[0].every(
            (cell, idx) =>
              String(cell ?? '') === String(tableItem.columns[idx] ?? '')
          );
    ...
    const height =
      typeof imageItem.height === 'number' ? imageItem.height : null;
